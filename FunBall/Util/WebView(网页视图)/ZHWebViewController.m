...
- (void)configBackItem {
    ...
    // 导航栏的返回按钮
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem itemWithTarget:self action:@selector(backBtnPressed:) image:@"back" highImage:@"back"];
}
...
- (void)configColseItem {
    ...
    // 导航栏的关闭按钮
    UIBarButtonItem *colseItem =[UIBarButtonItem itemWithTarget:self action:@selector(colseBtnPressed:) image:@"close" highImage:@"close"];
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    NSLog(@"页面地址：%@ 标题:%@",URL,webView.title);
...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)),dispatch_get_main_queue(), ^{
        ...
        NSLog(@"标题***%@",webView.title);
        
        if (!([self.title isEqualToString:@"资金托管认证"]||[self.title isEqualToString:@"运营商授权"]||[self.title isEqualToString:@"电商授权"]||[self.title isEqualToString:@"芝麻授权"]||[self.title isEqualToString:@"社保授权"]||[self.title isEqualToString:@"公积金授权"]||[self.title isEqualToString:@"网银授权"]||[self.title isEqualToString:@"注册协议"])) {
