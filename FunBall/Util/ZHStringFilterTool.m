...
#pragma mark 时间拼接之后的时间字符串
+(NSString*)appendDateWithStartDate:(NSString*)startDate DelayDays:(NSString*)day{

    if (NULLString(day)||NULLString(startDate)) {
        
        return nil;
    }
    
    NSInteger seconds=[day integerValue]*24*3600;

    NSDate *nowDate = [[ZHStringFilterTool changeStringDate:startDate] dateByAddingTimeInterval:seconds];

    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];

    formatter.dateFormat = @"yyyy-MM-dd";

    NSString * endDate = [formatter stringFromDate:nowDate];

    return endDate;
}

#pragma mark 字符串替换成时间
+(NSDate*)changeStringDate:(NSString*)dateString{
